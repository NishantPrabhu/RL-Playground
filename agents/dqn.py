...
class GradientReversalDQN:
    ...
    def __init__(
        ...
        self.action_predictor = nn.Sequential(RevGrad(), nn.Linear(2 * enc_fdim, 2))
        self.ap_optim = torch.optim.RMSprop(
            list(self.action_predictor.parameters())+list(self.online_q.encoder.parameters()),
            lr=ap_learning_rate, weight_decay=1e-05, betas=(0.9, 0.999)
        )
    ...
    def save(self, out_dir):
        state = {'step': self.step, 'model_state': self.online_q.state_dict()}
        torch.save(state, os.path.join(out_dir, 'ckpt.pth'))
    ...
    def update_q_networks(self, batch):
        obs, action, reward, next_obs, done = batch 
        obs = self.preprocess_obs(obs)
        next_obs = self.preprocess_obs(next_obs)
        action = torch.from_numpy(action).long().to(self.device)
    ...
    def update_encoder(self, batch):
        ...
        perm = torch.randperm(obs.size(0)).to(obs.device)
        obs_, action_ = obs[perm], action[perm]

        z = self.online_q.encoder(self.preprocess_obs(obs))
        z_ = self.online_q.encoder(self.preprocess_obs(obs_))
        inputs = torch.cat([z, z_], 1)
        labels = action.eq(action_).long()
        
        logits = self.action_predictor(inputs)
